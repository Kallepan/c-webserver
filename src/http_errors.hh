...
public:
    ...
    virtual std::string defaultFileToServe() const = 0; // returns the default file to serve
    virtual std::string defaultContent() const = 0; // returns the default content if the file does not exist

    virtual int getStatusCode() const = 0;  // returns the status code
    virtual std::string getStatusMessage() const = 0; // returns the status message
};
...
public:
    ...
    std::string defaultFileToServe() const override {
        return "res/400.html";
    }
